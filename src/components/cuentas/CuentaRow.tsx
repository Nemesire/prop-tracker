...
export default function CuentaRow({ account: acc }: Props) {
  ...
  return (
    <>
      <tr className="border-b border-[#2D2D4E] hover:bg-[#22223A]/40 transition-colors group">
        <td className="px-5 py-4">
          <span className="font-mono text-sm text-[#F8F8FF]">{acc.name}</span>
        </td>
        <td className="px-5 py-4">
        ...
        <td className="px-5 py-4 text-sm text-[#8888AA]">{acc.company}</td>
        ...
        <td className="px-5 py-4">
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            {acc.type === 'evaluacion' && acc.status === 'activa' && (
              <button title="Renovar" onClick={() => updateAccount(acc.id, { cost: acc.cost })} className="p-1.5 rounded-lg hover:bg-[#22223A] text-[#8888AA] hover:text-[#F8F8FF] transition-colors">
                <RefreshCw size={14} />
              </button>
            )}
            {acc.type === 'evaluacion' && acc.status === 'activa' && (
              <button title="Aprobar evaluación → Live" onClick={() => approveEvaluation(acc.id)} className="p-1.5 rounded-lg hover:bg-[#22223A] text-[#8888AA] hover:text-[#22C55E] transition-colors">
                <Check size={14} />
              </button>
            )}
            {acc.type === 'live' && acc.status === 'activa' && (
              <button title="Registrar retiro" onClick={() => setWithdrawOpen(true)} className="p-1.5 rounded-lg hover:bg-[#22223A] text-[#8888AA] hover:text-[#22C55E] transition-colors">
                <DollarSign size={14} />
              </button>
            )}
            <button title="Suspender" onClick={() => updateAccount(acc.id, { status: 'suspendida' })} className="p-1.5 rounded-lg hover:bg-[#22223A] text-[#8888AA] hover:text-[#EF4444] transition-colors">
              <Ban size={14} />
            </button>
            <button title="Editar" onClick={() => setEditOpen(true)} className="p-1.5 rounded-lg hover:bg-[#22223A] text-[#8888AA] hover:text-[#F8F8FF] transition-colors">
              <Pencil size={14} />
            </button>
          </div>
        </td>
