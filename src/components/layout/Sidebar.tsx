...
export default function Sidebar() {
  ...
  const sidebarBg = 'bg-surface border-r border-border'
  ...
  const linkIdle   = 'text-muted hover:bg-surface2 hover:text-text'
  ...
  return (
    <aside className={`relative flex flex-col h-screen ${sidebarBg} transition-all duration-300 flex-shrink-0 ${collapsed ? 'w-16' : 'w-60'}`}>
      ...
      {currentUser && (
        <div className={`border-t border-border p-3 ${collapsed ? 'flex flex-col items-center gap-2' : ''}`}>
          {!collapsed ? (
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <div className="min-w-0">
                  <div className="text-sm font-medium text-text truncate">{currentUser.displayName}</div>
                  <div className="text-xs text-muted truncate">{level?.icon} {level?.name}</div>
                </div>
              </div>
              ...
              <button
                ...
                className="flex items-center gap-2 text-xs text-muted hover:text-[#EF4444] transition-colors py-1 w-full"
              >
            ...
            <>
              <Avatar name={currentUser.displayName} src={currentUser.avatar} size="sm" level={currentUser.level} />
              <button onClick={handleLogout} className="text-muted hover:text-[#EF4444] transition-colors">
                <LogOut size={16} />
              </button>
            </>
          )}
      ...
      <button
        ...
        className="absolute -right-3 top-20 w-6 h-6 bg-surface border border-border rounded-full flex items-center justify-center text-muted hover:text-text hover:border-[#7C3AED] transition-colors z-10"
      >
