...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[#0F0F1A] text-[#F8F8FF]">
      {/* Navbar */}
      <nav className="border-b border-[#2D2D4E] px-6 py-4 flex items-center justify-between">
      ...
      {/* Stats */}
      <section className="px-6 py-12 border-y border-[#2D2D4E]">
      ...
      <section className="px-6 py-20">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-[#8888AA] text-lg">Herramientas profesionales combinadas con la motivación de la comunidad</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {FEATURES.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="bg-[#1A1A2E] border border-[#2D2D4E] rounded-2xl p-6 hover:border-[#7C3AED]/40 hover:bg-[#1E1E38] transition-all group">
      ...
      {/* Testimonials */}
      <section className="px-6 py-20 bg-[#1A1A2E]/50">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {TESTIMONIALS.map(({ name, level, badge, text }) => (
              <div key={name} className="bg-[#1A1A2E] border border-[#2D2D4E] rounded-2xl p-5">
                <div className="text-3xl mb-4">"</div>
                <p className="text-[#8888AA] text-sm leading-relaxed mb-5">{text}</p>
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <div className="text-sm font-medium text-[#F8F8FF]">{name}</div>
                    <div className="text-xs text-[#8888AA]">{badge} {level}</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="px-6 py-24 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-[#8888AA] text-lg mb-8">Únete a miles de traders que ya gestionan sus prop firms con PropTracker</p>
      ...
      {/* Footer */}
      <footer className="border-t border-[#2D2D4E] px-6 py-8 text-center text-sm text-[#8888AA]">
        <div className="flex items-center justify-center gap-2 mb-3">
          ...
          <span className="font-semibold text-[#F8F8FF]">PropTracker</span>
        </div>
