          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {accounts.flatMap(a => a.withdrawalsList.map(w => ({ ...w, accountName: a.name, company: a.company }))).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(w => (
                <tr key={w.id} className="border-b border-border hover:bg-surface2/50">
                  <td className="px-5 py-3 text-muted text-xs">{new Date(w.date).toLocaleDateString('es-ES')}</td>
                  ...
                  <td className="px-5 py-3 text-muted text-xs">{w.note ?? '—'}</td>
                </tr>
              ))}
              {accounts.every(a => a.withdrawalsList.length === 0) && (
                <tr><td colSpan={5} className="px-5 py-8 text-center text-muted text-sm">Sin retiros registrados</td></tr>
              )}
            </tbody>
          </table>
        </div>
